/**
 * @file test_nsga2_zdt4.cpp
 * @brief 测试NSGA-II在ZDT4问题上的性能
 * @version 0.1
 * @date 2026-02-25
 * @example ./bin/Release/test_nsga2_zdt4.exe
 ...
 * @brief ZDT4问题
 */
class ZDT4Problem: public IMultiObjectiveProblem{
private:
    int m_numVariables = 10;
    int m_numObjectives = 2;
public:
    ZDT4Problem() = default;
    ~ZDT4Problem() override = default;
    ...
    void getBounds(cv::Mat& lowerBounds, cv::Mat& upperBounds) const override{
        lowerBounds = cv::Mat::ones(1, m_numVariables, CV_64FC1) * -5.0;
        upperBounds = cv::Mat::ones(1, m_numVariables, CV_64FC1) * 5.0;
        lowerBounds.col(0) = cv::Mat::ones(1, 1, CV_64FC1) * 0.0;
        upperBounds.col(0) = cv::Mat::ones(1, 1, CV_64FC1) * 1.0;
    }
    ...
    void evaluate(cv::InputArray populationVars, cv::OutputArray out_populationObjs) const override{
        ...
        for(int i = 0; i < N; i++){
            ...
            for(int j = 1;j < m_numVariables; ++j){
                sum += (x[j] * x[j] - 10.0 * cos(4.0 * M_PI * x[j]));  //Rastrigin多峰函数
            }
            double g = 1.0 + 10.0 * (m_numVariables - 1.0) + sum;
...
int main(int argc, char* argv[]) {
    argparse::ArgumentParser parser("test_nsga2_zdt4");
    ...
    //问题
    auto problem = std::make_shared<ZDT4Problem>();
