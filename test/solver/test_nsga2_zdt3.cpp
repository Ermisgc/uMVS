/**
 * @file test_nsga2_zdt3.cpp
 * @brief 测试NSGA-II在ZDT3问题上的性能
 * @version 0.1
 * @date 2026-02-25
 * @example ./bin/Release/test_nsga2_zdt3.exe
 ...
 * @brief ZDT3问题
 */
class ZDT3Problem: public IMultiObjectiveProblem{
private:
    int m_numVariables = 30;
    int m_numObjectives = 2;
public:
    ZDT3Problem() = default;
    ~ZDT3Problem() override = default;
    ...
    void evaluate(cv::InputArray populationVars, cv::OutputArray out_populationObjs) const override{
        ...
        for(int i = 0; i < N; i++){
            ...
            f[1] = g * (1.0 - std::sqrt(f[0] / g) - f[0] * sin(10.0 * M_PI * f[0]) / g); //函数2：f2(x) = g(x) * (1 - sqrt(f1(x) / g(x)) - f1(x) * sin(10.0 * M_PI * f1(x)) / g(x))
        }
...
int main(int argc, char* argv[]) {
    argparse::ArgumentParser parser("test_nsga2_zdt3");
    ...
    //问题
    auto problem = std::make_shared<ZDT3Problem>();
