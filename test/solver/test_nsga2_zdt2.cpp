/**
 * @file test_nsga2_zdt2.cpp
 * @brief 测试NSGA-II在ZDT2问题上的性能
 * @version 0.1
 * @date 2026-02-25
 * @example ./bin/Release/test_nsga2_zdt2.exe
 ...
 * @brief ZDT2问题
 */
class ZDT2Problem: public IMultiObjectiveProblem{
private:
    int m_numVariables = 30;
    int m_numObjectives = 2;
public:
    ZDT2Problem() = default;
    ~ZDT2Problem() override = default;
    ...
    void evaluate(cv::InputArray populationVars, cv::OutputArray out_populationObjs) const override{
        ...
        for(int i = 0; i < N; i++){
            ...
            f[1] = g * (1.0 - std::pow(f[0] / g, 2.0)); //函数2：f2(x) = g(x) * (1 - (f1(x) / g(x))^2)
        }
...
int main(int argc, char* argv[]) {
    argparse::ArgumentParser parser("test_nsga2_zdt2");
    ...
    //问题
    auto problem = std::make_shared<ZDT2Problem>();
