/**
 * @file test_nsga2_zdt6.cpp
 * @brief 测试NSGA-II在ZDT6问题上的性能
 * @version 0.1
 * @date 2026-02-25
 * @example ./bin/Release/test_nsga2_zdt6.exe
 ...
 * @brief ZDT4问题
 */
class ZDT6Problem: public IMultiObjectiveProblem{
private:
    int m_numVariables = 10;
    int m_numObjectives = 2;
public:
    ZDT6Problem() = default;
    ~ZDT6Problem() override = default;
    ...
    void evaluate(cv::InputArray populationVars, cv::OutputArray out_populationObjs) const override{
        ...
        for(int i = 0; i < N; i++){
            ...
            f[0] = 1 - exp(-4.0 * x[0]) * std::pow(sin(6.0 * M_PI * x[0]), 6); //函数1：f1(x) = 1 - exp(-4.0 * x[0]) * sin(6.0 * M_PI * x[0])
            double sum = 0.0;
            for(int j = 1;j < m_numVariables; ++j){
                sum += x[j];
            }
            double g = 1.0 + 9.0 * std::pow(sum / 9, 0.25);
            f[1] = g * (1.0 - pow(f[0] / g, 2.0)); //函数2：f2(x) = g(x) * (1 - pow(f1(x) / g(x), 2.0))
        }
...
int main(int argc, char* argv[]) {
    argparse::ArgumentParser parser("test_nsga2_zdt6");
    ...
    //问题
    auto problem = std::make_shared<ZDT6Problem>();
